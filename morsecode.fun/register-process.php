...
if (isset($_POST['submit'])) {
  $name = $_POST['username'];
  $pwd = $_POST['password'];
  $repeatpwd = $_POST['repeat'];
  if (empty($name) || empty($pwd) || empty($repeatpwd)) {
    echo("Hol' up!\nYou didn't enter in some boxes!");
  } else {
    if ($pwd == $repeatpwd) {
      $sql = mysqli_query($conn, "SELECT username FROM users WHERE username = '$name'");
      if (mysqli_num_rows($sql) > 0) {
        echo("Username taken!");
      } else {
        $hashedpwd = password_hash($pwd, PASSWORD_DEFAULT);
        $sql = "INSERT INTO users (username, password) VALUES ($name, $hashedpwd)";
      ...
      echo("Hol' up!\nYou didn't repeat the password properly!");
    }
  }
} else {
